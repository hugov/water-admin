	...
	<body>
		...
		<script lang="javascript">
			...
			function defineId(param) {
				var produtoEl = document.querySelector('button#produtoId');
				produtoEl.setAttribute('onclick', 'sendFormDelete(' + param + ')');
			}
		
			function sendFormDelete(id) {
			    $.ajax({
			        url: "produtoDelete.action",
			        type: "POST",
			        data: {'id': id},
			    	success: function() {
			    		location = 'produtoList.action';
			        	location.load();
			    	}
			    });
			}
			...
			<div class="modal-dialog" role="document">
				<div class="modal-content">
					...
					<div class="modal-footer">
						<button type="button" class="btn btn-primary" id="produtoId" onClick="sendFormDelete(<s:property value="id" />);">Sim</button>
		...
		<div id="main" class="container-fluid">
			...
			<div id="top" class="row">
				...
				<div class="col-md-3">
					<a href="produtoPrepareCreate.action" class="btn btn-primary pull-right h2">Adicionar Produto</a>
				</div>
			</div>
				...
				<div class="table-responsive col-md-12">
					<table class="table table-striped" cellspacing="0" cellpadding="0">
						...
						<tbody>
							...
							<s:iterator value="produtoList" status="produto">
							<tr>
								<td><s:property value="id" /></td>
								<td><s:property value="nome" /></td>
								<td><s:property value="categoria.descricao" /></td>
								<td><s:property value="preco" /></td>
								<td><s:if test="situacao == 1">Ativo</s:if><s:else>Inativo</s:else></td>
								<td class="actions">
									<a class="btn btn-success btn-xs" href="produtoRetrieve.action?id=<s:property value="id" />">Visualizar</a> 
									<a class="btn btn-warning btn-xs" href="produtoUpdate.action?submitType=updatedata&id=<s:property value="id" />">Editar</a> 
